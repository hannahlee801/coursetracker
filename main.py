...
from schemas import CreateStudentRequest, CreateInstructorRequest, CreateCourseRequest, AddStudentToCourseRequest
...
async def add_student_to_course(course_id: int, request: AddStudentToCourseRequest, db: Session = Depends(get_db)) -> None:
    ...
    if student in course.students:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Student already in course.")
